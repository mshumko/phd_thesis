...
Assume a uniform magnetic field $\vec{B} = B_0 \hat{z}$ with a paralle propagating ($k = k\hat{z}$), right-hand circularly polarized wave
...
\end{equation} where $\theta$ is the particle phase. Now the electron is traveling in the $-\hat{z}$ direction with a velocity $\vec{v} = -v_0 \hat{z}$ and its time dependent position along $\hat{z}$ and gyrophase can be expressed as 
...
\omega + kv_0 - \Omega = 0
\end{equation}, the electron will be in a static electric field which will accelerate or decelerate the electron depending on $\theta_0$, the phase between the wave and the electron. \textcolor{red}{Show Bortnik plot?} The expression in Eq. \ref{Intro:resonance} is commonly known as the resonance equation.
