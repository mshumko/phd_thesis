...
The short microburst duration observed by a single LEO satellite has an ambiguity when interpreting what is exactly a microburst. The two possible realities are: a microburst is very small and spatially stationary so that a LEO spacecraft passes through it in less than a second. Alternatively, a microburst is spatially large and transient. There are a few ways to distinguish between the two possible realities, and each one has a unique set of advantages.
